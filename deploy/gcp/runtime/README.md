...
the dispatch broker and deterministic workers join this root only after their
own security gates pass.
...
export IMAGE="${REGION}-docker.pkg.dev/${PROJECT_ID}/${REPOSITORY}/attune-audit-writer"

docker buildx build --platform=linux/amd64 --push \
  -f deploy/audit-writer/Dockerfile -t "${IMAGE}:audit-writer-v1" .
gcloud artifacts docker images describe "${IMAGE}:audit-writer-v1" \
  --project="$PROJECT_ID" \
...
Verify that ingress is internal, unauthenticated invocation fails, the IAM
policy lists only the four expected workload identities, and the audit-writer
service account has no user-managed key. Do not place tenant data, tokens, or
credentials in Terraform variables, state, labels, probes, or deployment logs.
