...
A live variant (real Mem0 + Qdrant + Fuel iX) runs the same knowledge-update
shape when ``ADC_LIVE_MEMORY_EVAL=1`` — manual, after memory-pipeline
changes, never in CI.
"""
...
from aidedecamp.memory.mem0_store import Mem0Store
...
def test_signal_cap_bounds_the_prompt():
    from aidedecamp.memory.mem0_store import CONSOLIDATE_SIGNAL_CAP
...
# ---------------------------------------------------------------------------
# Live variant — real Mem0/Qdrant/Fuel iX; manual, never CI
# ---------------------------------------------------------------------------


@pytest.mark.skipif(
    os.environ.get("ADC_LIVE_MEMORY_EVAL") != "1",
    reason="live memory eval: set ADC_LIVE_MEMORY_EVAL=1 (needs Qdrant + FUELIX_TOKEN)",
)
def test_live_knowledge_update():  # pragma: no cover - live services
    from aidedecamp.fuelix import make_client
    from aidedecamp.memory.mem0_store import build_mem0_config
